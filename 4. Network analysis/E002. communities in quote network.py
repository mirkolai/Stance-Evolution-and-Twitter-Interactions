...
cur.execute("""CREATE TABLE IF NOT EXISTS `user_quote_relation_communities` (
...
for phase in range(1,5):
    ...
    cur.execute("""
    SELECT `source`, `target` FROM `user_quote_relation` where phase=%s
    """,(phase))
    edges=cur.fetchall()
    i=len(edges)
    ...
    for key,value in partition.items():
        ...
        cur.execute(" INSERT INTO "
                    " `user_quote_relation_communities`"
                    " (`id`, `community`,phase) "
                    " VALUES "
